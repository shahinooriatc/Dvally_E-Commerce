import { Button, Card, Col, Container, Row, Spinner, Badge, Table, Modal, ListGroupItem, Form } from "react-bootstrap";
...
const Products = () => {
  ...
  //Product Cart Here.........
  const { state, dispatch: contDispatch, stateWish, dispatchWish } = useContext(Store)
  const { cart } = state
      ...
      <Container>
        <Form>
          <Form.Group className="my-1">
            <Form.Control onChange={handleSearch} bg='info' type="text" placeholder="Search Product" />
          </Form.Group>
        </Form>
        <Row>
          {loading ? (
            ...
            products.filter((item) => {
              ...
              .map((item) => (
                <Col key={item._id} lg={3} style={{ marginTop: "10px", height: "520px" }}>
                  <Card>
                    <Card.Img className="productImg" variant="top" src={item.img} />
                    ...
                    <Card.Body className='text-center'>
                      ...
                      <Card.Title style={{ color: 'goldenRod' }}>
                        <Rating rating={item.rating} totalrating={item.totalrating} />
                      </Card.Title>
                      <Card.Title>
                        <h5>$ {item.price}/-</h5>
                      </Card.Title>
                        ...
                        {cart.cartItems.map(items => (
                          items._id == item._id ?
                            <tbody key={items._id}>
                              <tr>
                                <td>
                                  <Button onClick={() => handleUpdateCartItem(items, items.quantity - 1)} variant="outline-danger" disabled={items.quantity === 1} >-</Button>
                                  <Badge>
                                    <span style={{ margin: '5px' }}>
                                      {items.quantity}
                                    </span>
                                  </Badge>
                                  <Button onClick={() => handleUpdateCartItem(items, items.quantity + 1)} variant="outline-success" disabled={items.quantity == item.stock}>+</Button>
                                </td>
                                <td>
                                  <Button onClick={() => handleRemoveItem(items)} variant="danger">Delete</Button>
                                </td>
                              </tr>
                            </tbody>
                            :
                      ...
                      {item.stock <= 0 ?
                        <>
                          ...
                          <Button onClick={() => handleDetails(item.slug)} variant='outline-info' className='mt-1 '>Details</Button>
                        ...
                        :
                        <>
                          <Button onClick={() => handleAddToCart(item)} className='mt-1 w-50'>Add to Cart</Button>
                          <Button onClick={() => handleDetails(item.slug)} variant='outline-info' className='mt-1'>Details</Button>
                          <Button variant="outline-danger" onClick={() => handleAddToWish(item)}>
                            <BsFillHeartFill />
                          </Button>
              ...
              <Row className="mt-3">
                  ...
                  <Card className="text-center">
                    <Card.Body>
                      ...
                      <ListGroupItem>
                        Rating :
                        <small>
                          <Rating rating={details.rating} totalrating={details.totalrating} />
                        </small>
                      </ListGroupItem>
                      ...
                      <Link to={""}>
                        {details.stock <= 0 ?
                          <>
                            <Button variant='danger' disabled className='mt-1'>Out Of Stock</Button>
                          </>
                          :
                ...
                <Col md={12}>
                  <Card>
                    <Card.Body>
                      <Link to="/">
                        <Button variant="info" className="w-50">Continue Shopping </Button>
                      </Link>
                      <Link to="/" className="w-100">
                        <Button variant="outline-primary" className="w-50"> Place Order</Button>
                      </Link>
                    </Card.Body>
                  </Card>
                </Col>
